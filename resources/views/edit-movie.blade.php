...
<body style="background: rgb(78, 78, 78)">
    ...
    <form class="m-auto mt-20 w-[22%] bg-white rounded-lg flex flex-col items-center" method="POST" action="{{route('update-movie', $movie)}}">
        ...
        <div class="flex flex-col items-center">
            <input value="{{$movie->getTranslation('title', 'en')}}" placeholder="Update Movie Name(en)" class="2xl:text-3xl border-blue-700 border-b-[2px] focus:outline-none w-[80%] ml-[5%]" type="text" name="title_en" id="title_en">
        </div>
        @error('title_en')
        ...
        <div class="flex flex-col items-center">
            <input value="{{$movie->getTranslation('title', 'ka')}}" placeholder="Update Movie Name(ka)" class="2xl:text-3xl border-blue-700 border-b-[2px] focus:outline-none w-[80%] ml-[5%]" type="text" name="title_ka" id="title_ka">
        </div>
        @error('title_ka')
        <p class="text-red-500 mt-2 2xl:text-3xl text-xl text-center">{{ $message }}</p>
        @enderror
